...
print(f"Floor energy %:     {floor_rays['total_energy'].sum()/df[band_cols].sum().sum()*100:.1f}%")
...
# Right: log x-axis
data = floor_rays['total_energy'].values
data = data[data > 0]
...
fig3.suptitle(f'Floor Ray Energy Distribution ({len(floor_rays)} rays) — scored by energy/distance')
fig3.tight_layout()
